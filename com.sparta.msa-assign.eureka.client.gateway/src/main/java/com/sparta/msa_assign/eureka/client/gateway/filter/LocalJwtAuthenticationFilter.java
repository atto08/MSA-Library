package com.sparta.msa_assign.eureka.client.gateway.filter;

import com.sparta.msa_assign.eureka.client.gateway.auths.AuthService;
...
public class LocalJwtAuthenticationFilter implements GlobalFilter {
    ...
    private boolean validateToken(String token) {
        try {
            ...
            if (claimsJws.getPayload().get("member_id") != null) {
                // user_id 추출 로직
                String memberId = claimsJws.getPayload().get("member_id").toString();
                // user_id 값으로 해당 유저가 회원가입 한 유저인지 인증 서비스를 통해 확인합니다.
                return authService.verifyMember(memberId);
            } else {
                return false;
            }
        }catch (Exception e) {
